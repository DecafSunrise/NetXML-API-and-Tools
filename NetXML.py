...
def makeCSV(FileIn, FileOut):

    # Parse NetXML file
    netxml = NetXML.iterparse(FileIn)

    # Set up the CSV writer
    with open(FileOut, "w", newline='') as FileOutput:
        ...
        for wn in netxml:
            if isinstance(wn, NetXML.WirelessNetwork):
                ...
                output.writerow((wn.netxml_type,wn.number,"",wn.network_type,wn.bssid,wn.ssid.essid,wn.manuf,wn.channel,";".join(wn.freqmhz),wn.ssid.max_rate,wn.ssid.beaconrate,wn.ssid.cloaked,";".join(wn.ssid.encryption),wn.ssid.privacy,wn.ssid.cipher,wn.ssid.authentication,wn.ssid.wpa_version,wn.ssid.wps,wn.ssid.frame_type,"","",wn.first_time, wn.last_time,wn.maxseenrate,wn.ssid.packets,wn._packets.llc,wn._packets.data,wn._packets.crypt,wn._packets.fragments,wn._packets.retries,wn._packets.total,wn.datasize,wn._gps.min_lat,wn._gps.min_lon,wn._gps.min_alt,wn._gps.min_spd,wn._gps.max_lat,wn._gps.max_lon,wn._gps.max_alt,wn._gps.max_spd,wn._gps.peak_lat,wn._gps.peak_lon,wn._gps.peak_alt,wn._gps.avg_lat,wn._gps.avg_lon,wn._gps.avg_alt))
            if isinstance(wn, NetXML.WirelessClient):
                if current_network_mac == wn.client_mac:
                    continue
                output.writerow((wn.netxml_type,wn.network_number,wn.number,wn.type,wn.client_mac,"",wn.client_manuf,wn.channel,";".join(wn.freqmhz),"","","","","","","","","","",wn.carrier,wn.encoding,wn.first_time, wn.last_time,wn.maxseenrate,"",wn._packets.llc,wn._packets.data,wn._packets.crypt,wn._packets.fragments,wn._packets.retries,wn._packets.total,wn.datasize,wn._gps.min_lat,wn._gps.min_lon,wn._gps.min_alt,wn._gps.min_spd,wn._gps.max_lat,wn._gps.max_lon,wn._gps.max_alt,wn._gps.max_spd,wn._gps.peak_lat,wn._gps.peak_lon,wn._gps.peak_alt,wn._gps.avg_lat,wn._gps.avg_lon,wn._gps.avg_alt))
